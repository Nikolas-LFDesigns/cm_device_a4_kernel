...
#if defined(CONFIG_MACH_ACER_A5)
#include "board-a5.h"
#ifdef CONFIG_MCU_MSP430
#include <linux/msp430.h>
#endif
#ifdef CONFIG_LEDS_MSP430
...
#if defined(CONFIG_MACH_ACER_A4)
#define MSM_PMEM_SF_SIZE	0x1e00000	/* 30MiB */
#elif defined(CONFIG_MACH_ACER_A5)
#define MSM_PMEM_SF_SIZE	0x2800000	/* 40MiB */
#else
#define MSM_PMEM_SF_SIZE	0x1700000
#endif

#if defined(CONFIG_MACH_ACER_A4)
#define MSM_FB_SIZE		0x300000	/* framebuffer used */
#elif defined(CONFIG_MACH_ACER_A5)
...
#define PMIC_GPIO_BACKLIGHT	25	/* PMIC GPIO Number 26, For Backlight */
#define GPIO_SD_DET		43 /* GPIO Number 43 */
#define PMIC_GPIO_SDC4_EN	0  /* PMIC GPIO Number 1 */
#define PMIC_GPIO_AUX_SLEEP_CLK2	38  /* PMIC GPIO Number 39 */
#define PMIC_GPIO_GYRO_PWR	1  /* PMIC GPIO Number 2 */
#define PMIC_GPIO_GYRO_INT	10  /* PMIC GPIO Number 11 */
#else
#ifdef CONFIG_MACH_ACER_A4
#define PMIC_GPIO_SD_DET	36
#define PMIC_GPIO_SDC4_EN	24  /* PMIC GPIO Number 25 */
...
#define PMIC_GPIO_CAM_SHUTDOWN_8M	8  /* PMIC GPIO Number 9 */
#endif
#endif
...
#if defined(CONFIG_MACH_ACER_A4) || defined(CONFIG_MACH_ACER_A5)
extern void __init acer_ts_init(void);

static struct embedded_sdio_data atheros_ar6000_data = {
        .cis    = {
                .vendor         = 0x271,
                .device         = 0x301,
                .blksize        = 2048,
...
#ifdef CONFIG_MACH_ACER_A5
acer_hw_version_t a5_hw_ver;

void get_acer_hw_version(void)
{
	...
	if (acer_smem_flag == NULL) {
		pr_err("[board-msm7x30] alloc acer_smem_flag failed!\n");
		a5_hw_ver = ACER_HW_VERSION_INVALID;
	} else {
		a5_hw_ver = acer_smem_flag->acer_hw_version;
		pr_err("[board-msm7x30] hw_ver = %d\n", a5_hw_ver);
	}
}

int enable_lg_lcd_power(int bOnOff)
{
	struct vreg *vreg_analog;		/* Analog power input */
	struct vreg *vreg_digital;		/* Digital power input */

	int rc = 0;

	pr_err("[LG_LCDC] in %s, PowerOn/Off = %d\n", __func__, bOnOff);

	vreg_analog = vreg_get(NULL, "gp6");		/* L15 */
	rc = vreg_set_level(vreg_analog, 2800);
	if (!rc) {
		if (bOnOff != 0) {
		...
		pr_err("[LG_LCDC] vreg_analog vreg enable/disable failed\n");
		return 0;
	}
	if (rc)
		pr_err("[LG_LCDC] %s set AVDD error, rc=%d\n", __func__, rc);
	else
		pr_err("[LG_LCDC] %s GP6 AVDD = 2.8V\n", __func__);


	vreg_digital = vreg_get(NULL, "gp7");		/* L8 */
	rc = vreg_set_level(vreg_digital, 1800);
	if (!rc) {
		if (bOnOff != 0) {
			rc = vreg_enable(vreg_digital);
		} else
			rc = vreg_disable(vreg_digital);
	} else {
		pr_err("[LG_LCDC] vreg_digital vreg enable/disable failed\n");
		return 0;
	}
	if (rc)
		pr_err("[LG_LCDC] %s set VDDI error, rc=%d\n", __func__, rc);
	else
		pr_err("[LG_LCDC] %s GP7 VDDI = 1.8V\n", __func__);

	return 0;
}
#endif	/* CONFIG_MACH_ACER_A5  For Display */
static int pm8058_gpios_init(void)
{
	int rc;

#ifndef CONFIG_MACH_ACER_A5
...
#ifdef CONFIG_ACER_HEADSET
	struct pm8058_gpio headset_amp_drv = {
		.direction      = PM_GPIO_DIR_OUT,
		.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
		.output_value   = 0,
		.pull           = PM_GPIO_PULL_NO,
...
#endif
	struct pm8058_gpio hdmi_cec_clk = {
		.direction      = PM_GPIO_DIR_OUT,
		.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
		.output_value   = 0,
		.pull           = PM_GPIO_PULL_NO,
...
#endif
#endif	/* CONFIG_MACH_ACER_A5  For Display, Audio and HDMI*/
...
#ifdef CONFIG_MACH_ACER_A5
#ifdef CONFIG_MT9E013
	struct pm8058_gpio cam_8m_shutdown = {
		.direction      = PM_GPIO_DIR_OUT,
		.pull           = PM_GPIO_PULL_UP_1P5,
		.vin_sel        = 2,
		.function       = PM_GPIO_FUNC_NORMAL,
...
#else
#if defined(CONFIG_MACH_ACER_A5)
	rc = pm8058_gpio_config(PMIC_GPIO_BACKLIGHT, &backlight_drv);
	if (rc) {
		pr_err("%s PMIC_GPIO_BACKLIGHT config failed\n", __func__);
		return rc;
	}
#ifdef CONFIG_ACER_HEADSET
	rc = pm8058_gpio_config(PMIC_GPIO_HS_AMP, &headset_amp_drv);
	if (rc) {
		pr_err("%s PMIC_GPIO_HS_AMP config failed\n", __func__);
		return rc;
	}
#endif
	rc = pm8058_gpio_config(PMIC_GPIO_AUX_SLEEP_CLK2, &hdmi_cec_clk);
	if (rc) {
		pr_err("%s PMIC_GPIO_AUX_SLEEP_CLK2 config failed\n", __func__);
		return rc;
	}
#endif
#endif /* CONFIG_MACH_ACER_A5 For Display and Audio*/
...
#ifdef CONFIG_MACH_ACER_A5
#ifdef CONFIG_MT9E013
	rc = pm8058_gpio_config(PMIC_GPIO_CAM_SHUTDOWN_8M, &cam_8m_shutdown);
	if (rc) {
		pr_err("%s PMIC_GPIO_CAM_SHUTDOWN_8M config failed\n", __func__);
		return rc;
	}
#endif
#ifdef CONFIG_SENSORS_MPU3050
	if (a5_hw_ver >= ACER_HW_VERSION_DVT2) {
		rc = pm8058_gpio_config(PMIC_GPIO_GYRO_PWR, &gyro_pwr);
		if (rc) {
			pr_err("%s PMIC_GPIO_GYRO_PWR config failed\n", __func__);
			return rc;
		}
		gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(PMIC_GPIO_GYRO_PWR), 1);
	}
#endif
#endif
	return 0;
...
#ifdef CONFIG_LEDS_MSP430
static struct led_mcu_data a5_leds[] = {
	[0] = {
		.id     = LED_NEW_EVENT_ID,
		.control_flag = LED_NEW_EVENT,
		.brightness = LED_OFF,
		.ldev   = {
			.name = "new_event",
		},
	...
	[1] = {
		.id     = LED_BATTERY_CHARGE_ID,
		.control_flag = LED_BATTERY_CHARGE,
		.brightness = LED_OFF,
		.ldev   = {
			.name = "battery_charge",
		},
	...
	[2] = {
		.id     = LED_BATTERY_FULL_ID,
		.control_flag = LED_BATTERY_CHARGE_COMPLETED,
		.brightness = LED_OFF,
		.ldev   = {
...
static struct leds_msp430_platform_data msp430_leds = {
	.num_leds = ARRAY_SIZE(a5_leds),
	.leds = a5_leds,
};

static struct platform_device msp430_led_device = {
	.name   = "msp430-leds",
	.id     = 0,
	.dev    = {
		.platform_data	= &msp430_leds,
	},
};
#endif
...
static struct i2c_board_info msm_camera_boardinfo[] __initdata = {
	...
	{
		I2C_BOARD_INFO("s5k4e1gx", 0x6C >> 1),
	},
#endif
#endif
...
#if defined(CONFIG_MACH_ACER_A4) && defined(CONFIG_S5K4E1GX)
	GPIO_CFG(1,  0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* VCM */
	GPIO_CFG(18, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* Reset */
	GPIO_CFG(85, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* PDN */
#endif
...
#if defined(CONFIG_MACH_ACER_A5)
static uint32_t cam2m_off_gpio_table[] = {
	/* parallel CAMERA interfaces */
	...
	GPIO_CFG(14, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* VSYNC_IN */
	GPIO_CFG(15, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* MCLK */
	GPIO_CFG(86, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* 2M PDN */
};
#endif
...
#if defined(CONFIG_MACH_ACER_A5)
static uint32_t cam2m_on_gpio_table[] = {
	/* parallel CAMERA interfaces */
	GPIO_CFG(0,  0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RST */
	GPIO_CFG(4,  1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* DAT4 */
	GPIO_CFG(5,  1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* DAT5 */
...
#if defined(CONFIG_MACH_ACER_A5)
static int config_cam2m_on_gpios(void)
{
	config_gpio_table(cam2m_on_gpio_table,
		ARRAY_SIZE(cam2m_on_gpio_table));
	return 0;
}
#endif
...
#if defined(CONFIG_MACH_ACER_A5)
static void config_cam2m_off_gpios(void)
{
	config_gpio_table(cam2m_off_gpio_table,
		ARRAY_SIZE(cam2m_off_gpio_table));
}
#endif
...
#if defined(CONFIG_MACH_ACER_A5)
struct msm_camera_device_platform_data cam2m_device_data = {
	.camera_gpio_on  = config_cam2m_on_gpios,
	.camera_gpio_off = config_cam2m_off_gpios,
	.ioext.camifpadphy = 0xAB000000,
	.ioext.camifpadsz  = 0x00000400,
	.ioext.csiphy = 0xA6100000,
	.ioext.csisz  = 0x00000400,
...
static struct msm_camera_sensor_flash_src msm_flash_src = {
	...
	._fsrc.drv_ic_src.flash_en_gpio = 36,
	._fsrc.drv_ic_src.strobe_gpio = 88,
};
#endif
...
static struct msm_camera_sensor_info msm_camera_sensor_mt9e013_data = {
	.sensor_name    = "mt9e013",
#if defined(CONFIG_MACH_ACER_A5)
	.sensor_reset   = PM8058_GPIO_PM_TO_SYS(PMIC_GPIO_CAM_SHUTDOWN_8M),
#else
	.sensor_reset   = 0,
#endif
	.sensor_pwd     = 85,
...
static unsigned audio_gpio_on[] = {
	GPIO_CFG(GPIO_HS_DET, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),			/* GPIO_HS_DET */
	GPIO_CFG(GPIO_HS_BUTT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),			/* GPIO_HS_BUTT */
	GPIO_CFG(GPIO_HS_SWITCH, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_16MA),	/* GPIO_HS_SWITCH */
	GPIO_CFG(GPIO_SPK_AMP, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_16MA),		/* GPIO_SPK_AMP */
};
#elif defined(CONFIG_MACH_ACER_A5)
static unsigned audio_gpio_on[] = {
	GPIO_CFG(GPIO_HS_DET, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),			/* GPIO_HS_DET */
...
static int __init snddev_poweramp_gpio_init(void)
{
    ...
    for (pin = 0; pin < ARRAY_SIZE(audio_gpio_on); pin++) {
		rc = gpio_tlmm_config(audio_gpio_on[pin], GPIO_CFG_ENABLE);
		if (rc) {
			printk(KERN_ERR
...
#if defined(CONFIG_MACH_ACER_A4) || defined(CONFIG_MACH_ACER_A5)
void msm_snddev_spk_dolby_pamp_on(void)
{
#if defined(CONFIG_MACH_ACER_A4)
#ifdef CONFIG_AUDIO_TPA2051
	tpa2051_speaker_dolby_switch(1);
#endif
#elif defined(CONFIG_MACH_ACER_A5)
#ifdef CONFIG_AUDIO_TPA2028
	tpa2028_speaker_dolby_switch(1);
#endif
#endif
}

void msm_snddev_spk_dolby_pamp_off(void)
{
#if defined(CONFIG_MACH_ACER_A4)
#ifdef CONFIG_AUDIO_TPA2051
	tpa2051_speaker_dolby_switch(0);
#endif
#elif defined(CONFIG_MACH_ACER_A5)
#ifdef CONFIG_AUDIO_TPA2028
	tpa2028_speaker_dolby_switch(0);
#endif
#endif
}

void msm_snddev_spk_phone_pamp_on(void)
{
#if defined(CONFIG_MACH_ACER_A4)
#ifdef CONFIG_AUDIO_TPA2051
	tpa2051_speaker_phone_switch(1);
#endif
#elif defined(CONFIG_MACH_ACER_A5)
#ifdef CONFIG_AUDIO_TPA2028
	tpa2028_speaker_phone_switch(1);
#endif
#endif
}

void msm_snddev_spk_phone_pamp_off(void)
{
#if defined(CONFIG_MACH_ACER_A4)
#ifdef CONFIG_AUDIO_TPA2051
	tpa2051_speaker_phone_switch(0);
#endif
#elif defined(CONFIG_MACH_ACER_A5)
#ifdef CONFIG_AUDIO_TPA2028
	tpa2028_speaker_phone_switch(0);
#endif
#endif
}
...
void msm_snddev_hsed_spk_pamp_on(void)
{
#if defined(CONFIG_MACH_ACER_A4)
#ifdef CONFIG_AUDIO_TPA2051
	tpa2051_headset_speaker_switch(1);
#endif
#elif defined(CONFIG_MACH_ACER_A5)
#ifdef CONFIG_AUDIO_TPA2028
	tpa2028_speaker_dolby_switch(1);
...
void msm_snddev_hsed_spk_pamp_off(void)
{
#if defined(CONFIG_MACH_ACER_A4)
#ifdef CONFIG_AUDIO_TPA2051
	tpa2051_headset_speaker_switch(0);
#endif
#elif defined(CONFIG_MACH_ACER_A5)
#ifdef CONFIG_AUDIO_TPA2028
	tpa2028_speaker_dolby_switch(0);
...
void msm_snddev_mic1_pmic_on(void)
{
	pmic_hsed_enable(PM_HSED_CONTROLLER_0, PM_HSED_ENABLE_PWM_TCXO);
}

void msm_snddev_mic1_pmic_off(void)
{
	pmic_hsed_enable(PM_HSED_CONTROLLER_0, PM_HSED_ENABLE_OFF);
}

void msm_snddev_dualmic_pmic_on(void)
{
	pmic_hsed_enable(PM_HSED_CONTROLLER_0, PM_HSED_ENABLE_PWM_TCXO);
	pmic_hsed_enable(PM_HSED_CONTROLLER_2, PM_HSED_ENABLE_PWM_TCXO);
}

void msm_snddev_dualmic_pmic_off(void)
{
	pmic_hsed_enable(PM_HSED_CONTROLLER_0, PM_HSED_ENABLE_OFF);
...
#ifdef CONFIG_MACH_ACER_A4
static struct vreg *vreg_fm_l14;
#endif
static int fm_radio_setup(struct marimba_fm_platform_data *pdata)
{
...
#ifdef CONFIG_MACH_ACER_A4
	if (IS_ERR(vreg_fm_l14)) {
		printk(KERN_ERR "%s: vreg_fm get failed (%ld)\n",
			__func__, PTR_ERR(vreg_fm_l14));
		return -1;
	}
#endif
...
#ifdef CONFIG_MACH_ACER_A4
	rc = vreg_set_level(vreg_fm_l14,2600 );
	if(rc < 0){
		printk(KERN_ERR "%s: voltage level vote failed (%d)\n",
					__func__, rc);
		return -EIO;
...
#ifdef CONFIG_MACH_ACER_A4
	rc = vreg_enable(vreg_fm_l14);
	if (rc){
		printk(KERN_ERR "%s: vreg_fm_l14 enable failed(%d)\n",
					__func__, rc);
		return -EIO;
	}
#endif
	...
	irqcfg = GPIO_CFG(147, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
					GPIO_CFG_2MA);
#ifdef CONFIG_MACH_ACER_A4
	fm_lna_switch = GPIO_CFG(80, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP,
							GPIO_CFG_2MA);
#endif
...
#ifdef CONFIG_MACH_ACER_A4
	rc = gpio_tlmm_config(fm_lna_switch, GPIO_CFG_ENABLE);
	if (rc) {
		printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n",
				__func__, fm_lna_switch, rc);
		rc = -EIO;
		goto fm_gpio_config_fail;
...
#ifdef CONFIG_MACH_ACER_A4
	uint32_t fm_lna_switch = GPIO_CFG(80, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN,
							GPIO_CFG_2MA);
#endif
	int bahama_not_marimba = bahama_present();
	if (bahama_not_marimba == -1) {
...
static struct msm_hdmi_platform_data adv7525_hdmi_data = {
	.irq = MSM_GPIO_TO_INT(A5_N_HDMI_INT),
	.comm_power = hdmi_comm_power,
	.init_irq = hdmi_init_irq,
	.enable_5v = hdmi_enable_5v,
	.cec_power = hdmi_cec_power,
	.intr_detect = a5_hdmi_intr_detect,
};
#endif
#endif

#ifdef CONFIG_LEDS_TCA6507
static struct tca6507_pin_config tca6507_pin[] = {
	{
		.name = "call",
		.output_pin = 0,
	},
...
#ifdef CONFIG_MACH_ACER_A5
/* Gyro suport E-compass and G-sensor driver, this include format and select chip */
...
#if defined(CONFIG_MACH_ACER_A4) || defined(CONFIG_MACH_ACER_A5)
	{
		I2C_BOARD_INFO("smb136", 0x12),
	},
#endif
#if defined(CONFIG_MACH_ACER_A4)
#ifdef CONFIG_AUDIO_TPA2051
	{
		I2C_BOARD_INFO("tpa2051", 0x70),
	},
#endif
#ifdef CONFIG_BOSCH_SMB380
	{
...
#ifdef CONFIG_AUDIO_TPA2028
	{
		I2C_BOARD_INFO("tpa2028", 0x58),
	},
#endif
#ifdef CONFIG_MCU_MSP430
	{
		I2C_BOARD_INFO(MSP430_DRIVER_NAME, 0x68),
		.irq		=  MSM_GPIO_TO_INT(A5_MCU_IRQ),
	},
#endif
#if defined(CONFIG_SENSORS_MPU3050) || defined(CONFIG_SENSORS_MPU3050_MODULE)
	{
		I2C_BOARD_INFO(SENSOR_MPU_NAME, 0x69),
		.irq = PM8058_GPIO_IRQ(PMIC8058_IRQ_BASE, PMIC_GPIO_GYRO_INT),
...
#if (defined(CONFIG_MACH_ACER_A5) || defined(CONFIG_MACH_ACER_A4)) && defined(CONFIG_ADP1650_FLASH_DRIVER)
#ifdef CONFIG_ADP1650_FLASH_DRIVER
	{
...
#ifdef CONFIG_LEDS_TCA6507
	{
		I2C_BOARD_INFO(TCA6507_NAME, 0x45),
...
static struct hs_butt_gpio hs_butt_data = {
	.gpio_hs_butt = GPIO_HS_BUTT,
	.gpio_hs_dett = GPIO_HS_DET,
};
...
static struct msm_gpio dtv_panel_irq_gpios[] = {
	...
	{ GPIO_CFG(18, 0, GPIO_CFG_INPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
		"hdmi_int" },
#else
	{ GPIO_CFG(A5_N_HDMI_INT, 0, GPIO_CFG_INPUT,  GPIO_CFG_PULL_UP,
		GPIO_CFG_2MA), "hdmi_int" },
#endif
};
...
#else
	{ GPIO_CFG(A5_GPIO_VOUT_5V_EN, 0, GPIO_CFG_OUTPUT,  GPIO_CFG_PULL_DOWN,
		GPIO_CFG_2MA), "hdmi_5v_en" },
#endif
...
static int a5_hdmi_intr_detect(void)
{
	return gpio_get_value(A5_N_HDMI_INT);
}

static int hdmi_enable_5v(int on)
{
	pr_info("%s: %d\n", __func__, on);
	gpio_set_value(A5_GPIO_VOUT_5V_EN, on);
...
static int hdmi_cec_power(int on)
{
	pr_info("%s: %d\n", __func__, on);
	gpio_set_value_cansleep(
		PM8058_GPIO_PM_TO_SYS(PMIC_GPIO_AUX_SLEEP_CLK2), on);
...
#endif /* CONFIG_MACH_ACER_A4 For Display */


static struct mddi_platform_data mddi_pdata = {
#ifdef CONFIG_MACH_ACER_A4
	.mddi_power_save = NULL,
	.mddi_client_power = NULL,
#else
...
#ifdef CONFIG_MACH_ACER_A5
static struct msm_gpio lg_panel_gpios[] = {
	...
	{ GPIO_CFG(45, 0, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "spi_clk" },
	{ GPIO_CFG(44, 0, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "spi_cs1" },
	{ GPIO_CFG(47, 0, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "spi_mosi" },
	{ GPIO_CFG(90, 1, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "lcdc_pclk" },
	{ GPIO_CFG(91, 1, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "lcdc_en" },
	{ GPIO_CFG(92, 1, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "lcdc_vsync" },
	{ GPIO_CFG(93, 1, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "lcdc_hsync" },
	...
	{ GPIO_CFG(180, 0, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "lcd_reset" },
};

static struct msm_gpio lg_panel_gpios_sleep[] = {
	...
	{ GPIO_CFG(44, 0, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "spi_cs1" },
	{ GPIO_CFG(47, 0, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "spi_mosi" },
	{ GPIO_CFG(90, 0, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "lcdc_pclk" },
	{ GPIO_CFG(91, 0, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "lcdc_en" },
	{ GPIO_CFG(92, 0, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "lcdc_vsync" },
	{ GPIO_CFG(93, 0, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "lcdc_hsync" },
...
#else
#if defined(CONFIG_FB_MSM_LCDC_LG_WSVGA)
static int lcdc_lg_panel_power(int on)
{
	int rc = 0, i;
	static int current_lcdc_lg_power;
	struct msm_gpio *gp;

	if (current_lcdc_lg_power == on)
		return rc;

	current_lcdc_lg_power = on;

	/* Turn on/off lcd power */
	enable_lg_lcd_power(on);

	/* config RGB and SPI gpio */
	if (on) {
		rc = msm_gpios_enable(lg_panel_gpios, ARRAY_SIZE(lg_panel_gpios));
		if (rc < 0) {
			pr_err("%s: LG gpio enable failed: %d\n", __func__, rc);
		}
	} else {
		rc = msm_gpios_enable(lg_panel_gpios_sleep, ARRAY_SIZE(lg_panel_gpios_sleep));
		if (rc < 0) {
			pr_err("%s: LG sleep gpio enable failed: %d\n", __func__, rc);
		}
		gp = lg_panel_gpios_sleep;
		for (i = 0; i < ARRAY_SIZE(lg_panel_gpios_sleep); i++) {
			gpio_set_value(GPIO_PIN(gp->gpio_cfg), 0);
			gp++;
		}
	}
...
#endif	/* CONFIG_FB_MSM_LCDC_LG_WSVGA */
#endif	/* CONFIG_MACH_ACER_A5 */

static struct lcdc_platform_data lcdc_pdata = {
#ifdef CONFIG_MACH_ACER_A5
	.lcdc_power_save = lcdc_lg_panel_power,
#else
...
#ifdef CONFIG_MACH_ACER_A4
static struct vreg *vreg_ldo10;
static void __init a4_sensor_power_init(void)
{
	int rc;

	vreg_ldo10 = vreg_get(NULL, "gp4");
	if (IS_ERR(vreg_ldo10)) {
		pr_err("%s: vreg_ldo10 get failed (%ld)\n",
...
/*
 *  WiFi-chip ar6003 power sequence implement. It is called by atheros wifi driver while insmod/rmmod module.
 */

#ifdef CONFIG_MACH_ACER_A5
#define WIFI_PWD_L 37   /* ar6003 chip power up/down switcher */
#define WL_PWR_EN 38     /* used to switch 3.3v I/O supply */
#define WIFI_VREG_ID "wlan"
#endif
#ifdef CONFIG_MACH_ACER_A4
#define WIFI_PWD_L 124   /* ar6003 chip power up/down switcher */
#define WL_PWR_EN 68     /* used to switch 3.3v I/O supply */
#define WIFI_VREG_ID "wlan2"
#endif
bool wifi_power(int on)
{

	int rc;
	struct vreg *vreg_wlan;
	uint32_t gpio_pwd_cfg;
	uint32_t gpio_pwr_cfg;

	pr_info("+wifi_power(%s)\n", on?"on":"off");

	gpio_pwr_cfg =  GPIO_CFG(WL_PWR_EN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA);
	gpio_tlmm_config(gpio_pwr_cfg, GPIO_CFG_ENABLE);

	gpio_pwd_cfg =  GPIO_CFG(WIFI_PWD_L, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA);
	gpio_tlmm_config(gpio_pwd_cfg, GPIO_CFG_ENABLE);

	vreg_wlan = vreg_get(NULL, WIFI_VREG_ID);
	if (IS_ERR(vreg_wlan)) {
		pr_err("%s: vreg_get(%s) failed (%ld)\n",
		__func__, WIFI_VREG_ID, PTR_ERR(vreg_wlan));
		return false;
	}

	if (on == 1) {
		gpio_set_value(WIFI_PWD_L, 0);
		...
		if (rc) {
			pr_err("%s: vreg_enable(%s) failed (%d)\n", WIFI_VREG_ID, __func__, rc);
			return false;
		}
		udelay(200);
		gpio_set_value(WIFI_PWD_L, 1);
	} else {
		gpio_set_value(WIFI_PWD_L, 0);
		udelay(5);
...
struct vreg *vreg_gp10;
struct vreg *vreg_lvsw0;
#endif
...
static struct msm_gpio sdc2_sleep_cfg_data[] = {
	{GPIO_CFG(64, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), "sdc2_clk"},
	{GPIO_CFG(65, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), "sdc2_cmd"},
	{GPIO_CFG(66, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), "sdc2_dat_3"},
	{GPIO_CFG(67, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), "sdc2_dat_2"},
	{GPIO_CFG(68, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), "sdc2_dat_1"},
	{GPIO_CFG(69, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), "sdc2_dat_0"},

#ifdef CONFIG_MMC_MSM_SDC2_8_BIT_SUPPORT
	{GPIO_CFG(115, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), "sdc2_dat_4"},
	{GPIO_CFG(114, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), "sdc2_dat_5"},
	{GPIO_CFG(113, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), "sdc2_dat_6"},
	{GPIO_CFG(112, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), "sdc2_dat_7"},
#endif
};
#endif
...
static struct msm_gpio sdc4_sleep_cfg_data[] = {
	{GPIO_CFG(58, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), "sdc4_clk"},
	{GPIO_CFG(59, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), "sdc4_cmd"},
	{GPIO_CFG(60, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), "sdc4_dat_3"},
	{GPIO_CFG(61, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), "sdc4_dat_2"},
	{GPIO_CFG(62, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), "sdc4_dat_1"},
	{GPIO_CFG(63, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), "sdc4_dat_0"},
};
#endif
...
#if defined(CONFIG_MACH_ACER_A5) || defined(CONFIG_MACH_ACER_A4)
		if(dev_id == 4)
			gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(PMIC_GPIO_SDC4_EN), 1);
#endif
...
static unsigned int msm7x30_sdcc_slot_status(struct device *dev)
{
	return (unsigned int)
#ifdef CONFIG_MACH_ACER_A5
		(gpio_get_value_cansleep(GPIO_SD_DET) ? 0 : 1);
#else
#ifdef CONFIG_MACH_ACER_A4
		(gpio_get_value_cansleep(
			PM8058_GPIO_PM_TO_SYS(PMIC_GPIO_SD_DET - 1))?0:1);
#endif
		gpio_get_value_cansleep(
...
#ifdef CONFIG_MACH_ACER_A5
	.status_irq  = MSM_GPIO_TO_INT(GPIO_SD_DET),
#else
...
static void __init msm7x30_init_mmc(void)
{
	...
	if (IS_ERR(vreg_gp10)) {
		printk(KERN_ERR "%s: vreg get failed (%ld)\n",
		       __func__, PTR_ERR(vreg_gp10));
		return;
	}
	rc = vreg_set_level(vreg_gp10, 2850);
	if (rc)
		printk(KERN_ERR "%s: vreg_set_level() = %d \n", __func__, rc);
	...
	vreg_lvsw0 = vreg_get(NULL, "lvsw0");
	if (IS_ERR(vreg_lvsw0)) {
		printk(KERN_ERR "%s: vreg get failed (%ld)\n",
			__func__, PTR_ERR(vreg_lvsw0));
		return;
	}
	rc = vreg_set_level(vreg_lvsw0, 1800);
	if (rc)
		printk(KERN_ERR "%s: vreg_set_level() = %d \n", __func__, rc);

	rc = vreg_enable(vreg_lvsw0);
	if (rc)
		printk(KERN_ERR "%s: vreg_enable() = %d \n", __func__, rc);
#endif
#ifdef CONFIG_MMC_MSM_SDC1_SUPPORT
...
#if defined(CONFIG_BOARD_KRAMLOG_BASE) && defined(CONFIG_BOARD_KRAMLOG_SIZE)
	kramlog_memory_ready();
#endif
}

#if defined(CONFIG_MACH_ACER_A5)
MACHINE_START(ACER_A5, "jazz")
