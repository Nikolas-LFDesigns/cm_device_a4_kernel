...
#define DBG_MAX_WAITGPU_SECS (HZ/5)		// If the GPU execution time more than 0.2 seconds, we will show the actual execution time of GPU
#endif
...
#if defined(CONFIG_MACH_ACER_A5) || defined(CONFIG_MACH_ACER_A4)
	unsigned long debug_time = jiffies + DBG_MAX_WAITGPU_SECS;
	unsigned long save_time = jiffies;
#endif
...
#if defined(CONFIG_MACH_ACER_A5) || defined(CONFIG_MACH_ACER_A4)
			if( time_after(jiffies, debug_time) )
				KGSL_DRV_ERR("[Normal] spun too long waiting for RB to idle: %ld ms\n", (jiffies-save_time)*1000/HZ);
#endif
			KGSL_DRV_VDBG("return %d\n", 0);
			return 0;
		}
...
#if defined(CONFIG_MACH_ACER_A5) || defined(CONFIG_MACH_ACER_A4)
	if( time_after(jiffies, debug_time) )
		KGSL_DRV_ERR("[ERR] spun too long waiting for RB to idle: %ld ms\n", (jiffies-save_time)*1000/HZ);
#else
